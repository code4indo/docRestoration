"""
Find Top K Highest PSNR Samples with Length Constraint

This script evaluates the LOCKED test set and identifies the top K samples with the HIGHEST PSNR scores,
filtering for samples where the ground truth text length is greater than a specified threshold.

Usage:
    poetry run python dual_modal_gan/scripts/find_top_k_psnr_filtered.py \
        ...
        --output_dir results/top_5_psnr_long_text \
        --gpu_id 1 \
        --top_k 5 \
        --min_chars 10
"""
...
def find_top_k_psnr_filtered(args):
    """Find top K samples with HIGHEST PSNR in test set, filtered by text length."""
    
    print("="*80)
    print(f"🔍 FINDING TOP {args.top_k} HIGHEST PSNR SAMPLES (Min Length > {args.min_chars})")
    print("="*80)
    ...
    print("\n" + "="*80)
    print(f"📈 TOP {args.top_k} HIGHEST PSNR RESULTS (Length > {args.min_chars})")
    print("="*80)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results/top_k_psnr_filtered')
    parser.add_argument('--gpu_id', type=str, default='1')
    parser.add_argument('--top_k', type=int, default=5)
    parser.add_argument('--min_chars', type=int, default=10)
    
    args = parser.parse_args()
    find_top_k_psnr_filtered(args)
