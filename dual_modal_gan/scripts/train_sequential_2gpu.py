...
def main(args):
    # ✨ SEQUENTIAL 2-GPU SETUP: Manual device placement (NO distributed strategy)
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu_id
    
    # Parse GPU IDs
    gpu_ids = [int(x.strip()) for x in args.gpu_id.split(',')]
    num_gpus = len(gpu_ids)
    use_dual_gpu = (num_gpus >= 2)  # Enable sequential placement if 2+ GPUs available
    
    print(f"╔════════════════════════════════════════════════════════════════╗")
    print(f"║    Dual-Modal GAN-HTR Training (Sequential 2-GPU Strategy)    ║")
    print(f"╚════════════════════════════════════════════════════════════════╝")
    print(f"    Version: train_sequential_2gpu.py")
    print(f"    Precision: Pure FP32 (NO mixed precision)")
    print(f"    GPUs: {num_gpus}x NVIDIA RTX A4000 (IDs: {gpu_ids})")
    
    if use_dual_gpu:
        print(f"    Strategy: SEQUENTIAL GPU PLACEMENT")
        print(f"       GPU 0 ({'/device:GPU:0'}): Generator + Discriminator (trainable)")
        print(f"       GPU 1 ({'/device:GPU:1'}): Recognizer + VGG Perceptual Loss (frozen)")
        print(f"       Data flow: GPU0 → GPU1 (inference) → GPU0 (backprop)")
        print(f"    Expected speedup: ~1.3-1.5x (memory offloading + parallel inference)")
    else:
        print(f"    Strategy: Single GPU")
        print(f"       All models on GPU 0")
    print()
    ...
    print(f"   Samples per step: {args.batch_size} (no sharding)")
    ...
    # ✨ SEQUENTIAL 2-GPU: No dataset distribution needed
    # Data stays on GPU 0, only inference results are computed on GPU 1
    ...
    # ✨ SEQUENTIAL 2-GPU: Explicit device placement for each model
    
    # === GPU 0: Generator + Discriminator (trainable models) ===
    with tf.device('/GPU:0'):
        # --- Select Generator Architecture ---
        if args.generator_version == 'enhanced':
            print("   ✅ [GPU 0] Using ENHANCED generator (U-Net with Residual Blocks and Attention)")
            ...
            print("   🚀 [GPU 0] Using ENHANCED V2 generator (CBAM + RDB + Multi-Scale)")
            generator = unet_enhanced_v2(input_size=(1024, 128, 1))
            generator_name = "U-Net Enhanced V2 (CBAM+RDB+MSFP, 18.8M)"
        else:
            print("   ✅ [GPU 0] Using BASE generator (Standard U-Net)")
        ...
        if args.discriminator_version == 'enhanced_v2_fixed':
            ...
            print(f"   ✅ [GPU 0] Discriminator: ENHANCED_V2_FIXED (Reduced visual artifacts)")
            ...
            print(f"   ✅ [GPU 0] Discriminator: ENHANCED_V2 (18M params, ResNet + BiLSTM + Cross-Attention)")
        else:
            discriminator = build_dual_modal_discriminator(img_shape=(1024, 128, 1), vocab_size=vocab_size, max_text_len=128)
            print(f"   ✅ [GPU 0] Discriminator: BASE (137M params, Simple CNN + LSTM)")
    
    # === GPU 1: Recognizer (frozen, inference-only) ===
    recognizer_device = '/GPU:1' if use_dual_gpu else '/GPU:0'
    with tf.device(recognizer_device):
        # Load recognizer with multi-output for Recognition Feature Loss
        use_rec_feat_loss = args.rec_feat_loss_weight > 0.0
        print(f"   ✅ [{recognizer_device}] Loading Recognizer (frozen, inference-only)")
    ...
    print("   ✅ All models built with sequential GPU placement!")

    # ✨ SEQUENTIAL 2-GPU: Optimizers on GPU 0 (same as trainable models)
    with tf.device('/GPU:0'):
    ...
    # ✨ SEQUENTIAL 2-GPU: Loss functions and metrics (no distributed context needed)
    ...
    # ✨ SEQUENTIAL 2-GPU: Define device placement for train_step
    # These are closure variables accessible inside train_step
    gpu0_device = '/GPU:0'  # Generator, Discriminator (trainable)
    gpu1_device = '/GPU:1' if use_dual_gpu else '/GPU:0'  # Recognizer, VGG (frozen)

    # ✨ SEQUENTIAL 2-GPU: No @tf.function wrapper needed
    # Pure eager execution allows dynamic batch sizes and conditional logic
    # GPU placement is handled explicitly within train_step via tf.device()
    def train_step(degraded_images, clean_images, ground_truth_text, ctc_weight, rec_feat_weight, percep_weight):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            # ✨ SEQUENTIAL 2-GPU: Run recognizer inference on GPU 1 (if available)
            # Copy inputs to GPU 1, compute, then copy outputs back to GPU 0
            with tf.device(gpu1_device):
                # Copy image tensors to GPU 1
                clean_images_on_gpu1 = tf.identity(clean_images_normalized)
                generated_images_on_gpu1 = tf.identity(generated_images_normalized)
                
                # Run recognizer inference on GPU 1
                recognizer_output_clean_gpu1 = recognizer(clean_images_on_gpu1, training=False)
                recognizer_output_generated_gpu1 = recognizer(generated_images_on_gpu1, training=False)
            
            # Copy results back to GPU 0
            with tf.device(gpu0_device):
                recognizer_output_clean = tf.nest.map_structure(tf.identity, recognizer_output_clean_gpu1)
                recognizer_output_generated = tf.nest.map_structure(tf.identity, recognizer_output_generated_gpu1)
            ...
            # ✨ SEQUENTIAL 2-GPU: Run VGG Perceptual Loss on GPU 1 (if available)
            # Copy inputs to GPU 1, compute, then copy result back to GPU 0
            with tf.device(gpu1_device):
                # VGG Perceptual Loss - uses perceptual_loss_layer (Keras Layer)
                # Copy tensors to GPU 1 for computation
                clean_on_gpu1 = tf.identity(clean_images_tanh)
                generated_on_gpu1 = tf.identity(generated_images)
                perceptual_loss_gpu1 = perceptual_loss_layer(clean_on_gpu1, generated_on_gpu1)
            
            # Copy result back to GPU 0 for loss aggregation
            with tf.device(gpu0_device):
                perceptual_loss = tf.identity(perceptual_loss_gpu1)
    ...
    # ✨ SEQUENTIAL 2-GPU: No distributed wrapper needed
    # Call train_step directly (no strategy.run(), no reduction across replicas)
    # Data stays on GPU 0, only inference results cross to GPU 1
