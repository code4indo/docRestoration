...
# Enable Mixed Precision (FP16) for faster training and reduced memory usage
from tensorflow.keras import mixed_precision
mixed_precision.set_global_policy('mixed_float16')
print("🚀 Mixed Precision (FP16) enabled for performance optimization")
...
def run_validation_step(val_dataset, generator, recognizer, charset, psnr_metric, ssim_metric, cer_metric, wer_metric):
    ...
    for degraded_images, clean_images, labels in val_dataset:
        ...
        # FP16 Fix: Cast both to float32 for metric computation
        clean_images_f32 = tf.cast(clean_images, tf.float32)
        generated_images_f32 = tf.cast(generated_images, tf.float32)
        
        # Visual metrics: PSNR and SSIM expect values in [0, 1] range
        psnr = tf.image.psnr(clean_images_f32, generated_images_f32, max_val=1.0)
        ssim = tf.image.ssim(clean_images_f32, generated_images_f32, max_val=1.0)
        ...
        for i in range(labels_np.shape[0]):
            ...
            # Alternative: Use gt_text as reference for absolute HTR accuracy
...
def main(args):
    ...
    with strategy.scope():
        ...
        generator_optimizer_base = tf.keras.optimizers.Adam(args.lr_g, beta_1=0.5, clipnorm=args.gradient_clip_norm)
        # Use SGD for discriminator to save memory (Adam needs 2x memory for momentum states)
        discriminator_optimizer_base = tf.keras.optimizers.SGD(learning_rate=args.lr_d, momentum=0.9, clipnorm=args.gradient_clip_norm)
        
        # Wrap with LossScaleOptimizer for mixed precision training stability
        generator_optimizer = mixed_precision.LossScaleOptimizer(generator_optimizer_base)
        discriminator_optimizer = mixed_precision.LossScaleOptimizer(discriminator_optimizer_base)
        
        print(f"  Generator optimizer: Adam(lr={args.lr_g}, clipnorm={args.gradient_clip_norm}) + FP16 LossScaling")
        print(f"  Discriminator optimizer: SGD(lr={args.lr_d}, momentum=0.9, clipnorm={args.gradient_clip_norm}) + FP16 LossScaling")
    ...
    with strategy.scope():
        ...
        def train_step(degraded_images, clean_images, ground_truth_text):
            ...
            with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                ...
                # Cast to FP32 for mixed precision stability
                disc_loss_real = tf.cast(disc_loss_real, tf.float32)
                disc_loss_fake = tf.cast(disc_loss_fake, tf.float32)
                ...
                if args.ctc_loss_weight > 0:
                    ...
                    # FIXED: Raised from 300 to 1000 to allow proper learning
                ...
                # Cast all loss components to FP32 for mixed precision stability
                adversarial_loss = tf.cast(adversarial_loss, tf.float32)
                pixel_loss = tf.cast(pixel_loss, tf.float32)
                ctc_loss = tf.cast(ctc_loss, tf.float32)
            ...
            # Note: Clipping happens BEFORE LossScaleOptimizer's internal unscaling
            ...
            # LossScaleOptimizer will handle gradient unscaling internally
    ...
    experiment_name = f"GAN_HTR_{args.discriminator_mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    with mlflow.start_run():
        ...
        for epoch in range(args.epochs):
            ...
            for step in pbar:
                ...
                if args.ctc_loss_weight > 0:
                    epoch_metrics["losses"]["ctc_loss_raw"] = epoch_metrics["losses"].get("ctc_loss_raw", [])
            ...
            epoch_metrics["mean_losses"] = {
                "g_loss": float(np.mean(epoch_metrics["losses"]["g_loss"])),
                "d_loss": float(np.mean(epoch_metrics["losses"]["d_loss"])),
                "adv_loss": float(np.mean(epoch_metrics["losses"]["adv_loss"])),
                "l1_loss": float(np.mean(epoch_metrics["losses"]["l1_loss"])),
                "ctc_loss": float(np.mean(epoch_metrics["losses"]["ctc_loss"])),
                "ctc_loss_raw": float(np.mean(epoch_metrics["losses"].get("ctc_loss_raw", epoch_metrics["losses"]["ctc_loss"]))),
                "g_grad_norm_mean": float(np.mean(epoch_metrics["losses"]["g_grad_norm"])),
                "d_grad_norm_mean": float(np.mean(epoch_metrics["losses"]["d_grad_norm"])),
            ...
            mlflow_step = epoch + 1
            import sys
            print(f"  📊 [CODE_VERSION_v4_FINAL] MLflow logging: epoch={epoch}, step={mlflow_step}", flush=True)
            print(f"      VERIFY: epoch type={type(epoch)}, value={epoch}, step={mlflow_step}", flush=True)
            sys.stdout.flush()
            mlflow.log_metrics({
                "train/g_loss": epoch_metrics["mean_losses"]["g_loss"],
                "train/d_loss": epoch_metrics["mean_losses"]["d_loss"],
                "train/adv_loss": epoch_metrics["mean_losses"]["adv_loss"],
                "train/l1_loss": epoch_metrics["mean_losses"]["l1_loss"],
                "train/ctc_loss": epoch_metrics["mean_losses"]["ctc_loss"],
                "train/ctc_loss_raw": epoch_metrics["mean_losses"].get("ctc_loss_raw", epoch_metrics["mean_losses"]["ctc_loss"]),
                "train/g_grad_norm_mean": epoch_metrics["mean_losses"]["g_grad_norm_mean"],
                "train/d_grad_norm_mean": epoch_metrics["mean_losses"]["d_grad_norm_mean"],
                "train/g_grad_norm_max": epoch_metrics["mean_losses"]["g_grad_norm_max"]
            }, step=mlflow_step)  # Use epoch+1 for proper x-axis (1-100 instead of 0-99)
            ...
            if (epoch + 1) % args.eval_interval == 0:
                ...
                print(f"  Validation - PSNR: {psnr_result:.2f}, SSIM: {ssim_result:.4f}, CER: {cer_result:.4f}, WER: {wer_result:.4f}")
                ...
                if combined_score > best_val_psnr:
                    ...
                    print(f"  Current score {combined_score:.2f} not better than best {best_val_psnr:.2f}. No checkpoint saved.")
            ...
            if (epoch + 1) % args.save_interval == 0:
                ...
                for i in range(min(args.batch_size, 4)):  # Log max 4 samples to MLflow
                    img = img_to_save[i]
                    if img.shape[-1] == 1:
                ...
                print(f"  📊 Uploaded comparison samples to MLflow")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pixel_loss_weight', type=float, default=100.0, help='Weight for the L1 pixel loss (FIXED: 1000→100 to rebalance with CTC).')
    parser.add_argument('--ctc_loss_weight', type=float, default=1.0, help='Weight for the HTR CTC loss (FIXED: 15→1 to prevent dominance 99.8%).')
    ...
    parser.add_argument('--ctc_loss_clip_max', type=float, default=1000.0, help='Maximum value for CTC loss clipping (FIXED: 300→1000, observed max: 818).')
    ...
    parser.add_argument('--cer_weight', type=float, default=0.5, help='Weight for CER in combined score calculation (combined_score = PSNR - cer_weight * CER * 100). Default 0.5 means CER 0.1 (10%%) reduces score by 5 points.')
