"""
Find Top 10 Best Samples (Lowest CER, Highest PSNR) in Test Set

This script evaluates the LOCKED test set and identifies the top 10 samples that achieve:
...
def find_top_10_best_psnr_cer(args):
    """Find top 10 best samples (Lowest CER, then Highest PSNR)."""
    
    print("="*80)
    print(f"🔍 FINDING TOP 10 BEST SAMPLES (Lowest CER, Highest PSNR)")
    ...
    print(f"ℹ️  Collected {len(all_valid_samples)} valid samples")
    
    # Sort samples:
    # 1. CER Ascending (Lower is better)
    # 2. PSNR Descending (Higher is better)
    sorted_samples = sorted(all_valid_samples, key=lambda x: (x['cer'], -x['psnr']))
    
    # Take top 10
    top_10_samples = sorted_samples[:10]
    
    print("\n" + "="*80)
    print("🏆 TOP 10 BEST SAMPLES (Lowest CER, Highest PSNR)")
    print("="*80)
    
    os.makedirs(args.output_dir, exist_ok=True)
    results_summary = []
    
    for rank, sample in enumerate(top_10_samples, 1):
    ...
    # Save metadata
    meta_file = os.path.join(args.output_dir, 'top_10_best_psnr_cer_info.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results/top_10_best_psnr_cer')
    parser.add_argument('--gpu_id', type=str, default='1')
    parser.add_argument('--min_chars', type=int, default=10)
    
    args = parser.parse_args()
    find_top_10_best_psnr_cer(args)
