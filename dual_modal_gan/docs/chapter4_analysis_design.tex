...
\paragraph{Keuntungan Strategi Frozen Recognizer}

Strategi pembekuan memberikan keuntungan teknis dan operasional: (a) \textbf{stabilitas pelatihan} tanpa konflik gradien antara \textit{generator} dan \textit{recognizer}, (b) \textbf{efisiensi komputasi} dengan membekukan 27.86M parameter (pengurangan 61.6\% parameter \textit{trainable} vs \textit{joint training} yang melatih $\sim$45.3M parameter), (c) \textbf{konsistensi metrik} CER/WER sebagai evaluasi objektif, (d) \textbf{pencegahan \textit{catastrophic forgetting}} pada kemampuan pengenalan, dan (e) \textbf{optimasi terfokus} pada peningkatan kualitas visual citra untuk keterbacaan.

Validasi empiris (Bab~V.6.3) menunjukkan bahwa \textit{frozen recognizer} mempertahankan CER 31.63\% (degradasi moderat +5.06\% dari \textit{baseline} pra-pelatihan 26.57\%), sementara \textit{joint training} mengalami \textit{catastrophic forgetting} total dengan CER 100.00\% (degradasi +73.43\%), membuktikan keefektifan strategi pembekuan untuk domain dokumen paleografi historis.
