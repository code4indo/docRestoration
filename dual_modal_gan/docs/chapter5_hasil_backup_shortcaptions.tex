...
\renewcommand{\arraystretch}{1.3}
\centering
\caption{Perbandingan Indikatif dengan Metode \textit{State-of-the-Art} Berdasarkan Literatur}
...
\caption{Analisis korelasi tingkat degradasi pada \textit{set} validasi (n=710): (a) SNR vs peningkatan CER, (b) PSNR vs CER pascarestaorasi, (c) SSIM vs CER, (d) CER sebelum-sesudah.}
...
\begin{table}[H]
\renewcommand{\arraystretch}{1.15}
\caption{Distribusi Pola Kegagalan pada \textit{Test Set} ($n$=712)}
...
\begin{enumerate}[nosep]
    \item \textbf{Pipeline semi-otomatis} untuk restorasi dokumen paleografi dengan \textit{throughput} 4.500 dokumen/jam (inferensi GPU NVIDIA RTX A4000, waktu inferensi rata-rata 0.8 detik per citra), layak untuk digitalisasi skala operasional Arsip Nasional RI
