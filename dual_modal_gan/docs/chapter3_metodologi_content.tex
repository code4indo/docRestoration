...
\vspace{2cm}
\begin{center}
{\fontsize{14}{16.8}\selectfont\textbf{Bab III \quad Metodologi Penelitian}}\\[1em]
\end{center}
...
\item \textit{Kesenjangan Evaluasi---Optimasi Loss Function:} Tidak ada studi ablasi sistematis yang mengidentifikasi konfigurasi optimal untuk menyeimbangkan komponen funfsi loss (piksel, \textit{adversarial}, perseptual, CTC) guna mencapai keseimbangan Pareto antara kualitas visual dan keterbacaan HTR.
\end{enumerate}
...
\midrule
FR-4 & Pemrosesan Batch & Sistem harus mampu memproses dokumen terdegradasi secara batch dengan output terstruktur: (1) Gambar restorasi penuh, (2) File teks transkripsi (.txt), (3) File metadata evaluasi (.json) yang mencakup metrik PSNR, SSIM, CER, dan WER. \\
\midrule
FR-5 & Dukungan Dataset Sintetis & Sistem harus menyertakan alur otomatis untuk pembuatan dataset sintetis terdegradasi dari gambar bersih, memungkinkan augmentasi data untuk training dengan kontrol parameter degradasi. \\
\midrule
FR-6 & Output Terdefinisi & Format output konsisten mencakup citra restorasi (PNG/TIFF 300 DPI), transkripsi teks (UTF-8 dengan tingkat kepercayaan), metrik kualitas (PSNR/SSIM), dan penanganan kesalahan terstruktur. \\
\midrule
FR-7 & Input Data Fleksibel & Sistem menangani berbagai format input (JPG, PNG, TIFF, PDF) dengan validasi resolusi minimum, pemulihan kesalahan, dan pra-pemrosesan otomatis ke skala abu-abu. \\
\midrule
FR-8 & Integrasi Sistem & Sistem menyediakan berbagai antarmuka (CLI, Python API) dengan konfigurasi YAML/JSON dan log terstruktur untuk pemantauan. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{p{0.07\textwidth}p{0.25\textwidth}p{0.63\textwidth}}
\toprule
\textbf{Kode} & \textbf{Nama Requirement} & \textbf{Deskripsi dan Target Performa} \\
\midrule
NFR-1 & Kualitas Visual Restorasi & PSNR $>$ 30 dB, SSIM $\geq$ 0.90 pada validasi. \\
\midrule
NFR-2 & Peningkatan Keterbacaan Teks & Penurunan CER $\geq$ 25\% dari citra terdegradasi. \\
\midrule
NFR-3 & Performa Waktu Inferensi & Inferensi $<$ 15 detik/gambar (GPU RTX A4000). \\
\midrule
NFR-4 & Reproduktifitas & Reproduktif via \texttt{pyproject.toml} dan \textit{Docker}. \\
\bottomrule
\end{tabular}
\end{table}
...
Untuk memastikan kelengkapan dan konsistensi, Tabel~\ref{tab:traceability-matrix} menyajikan matriks pelacakan yang memetakan kebutuhan sistem ke rumusan masalah (Bab I), tujuan penelitian (Bab I), dan tahapan evaluasi (Bab V).
...
\midrule
FR-1 & Restorasi Gambar & RM-1, Tujuan T1 & V.2 (Metrik Visual) \\
\midrule
FR-2 & Penanganan Degradasi & RM-1, Tujuan T1 & V.2 (Metrik Kuantitatif) \\
\midrule
FR-3 & Integrasi HTR & RM-1, Tujuan T2 & V.2 (CER/WER) \\
\midrule
FR-4 & Pemprosesan Masal & Tujuan T4 & V.2 (Evaluasi Set Uji) \\
\midrule
FR-5 & Dataset Sintetis & Metodologi (Fase 3) & V.2 (Karakteristik Data) \\
\midrule
FR-6--FR-8 & Output \& Integrasi & Tujuan T4 & Implementasi (Bab IV) \\
\midrule
NFR-1 & Kualitas Visual & RM-1, Tujuan T1 & V.2 (PSNR/SSIM) \\
\midrule
NFR-2 & Keterbacaan Teks & RM-1, RM-3, Tujuan T2 & V.2 (CER/WER) \\
\midrule
NFR-3 & Performa Waktu & Implementasi Praktis & V.2\footnotemark[1] \\
\midrule
NFR-4 & Reproduktifitas & Metodologi DSRM & Konfigurasi MLflow \\
\bottomrule
\end{tabular}
\end{table}

\footnotetext[1]{Metrik waktu inferensi tidak dievaluasi secara terpisah namun divalidasi memenuhi target $<$15 detik per gambar pada GPU RTX A4000.}
...
% perintah menambahkan space sebelum paragraf
...
Model \textit{Recognizer} HTR dilatih menggunakan arsitektur \textit{hybrid} CNN-\textit{Transformer} dengan dataset ANRI paleografi Belanda. Setelah pelatihan mencapai konvergensi, bobot model dibekukan untuk memastikan konsistensi evaluasi selama pelatiahan GAN dan mencegah \textit{catastrophic forgetting}. Model beku ini diintegrasikan ke kerangka kerja untuk perhitungan CTC \textit{loss} yang di-\textit{backpropagate} ke Generator.
...
Karena keterbatasan ketersediaan data berpasangan untuk dokumen historis, dikembangkan alur degradasi sintetis yang realistis untuk menghasilkan \textit{training pairs} dari dokumen bersih. Alur ini mensimulasikan berbagai jenis degradasi karakteristik dokumen historis seperti \textit{bleed-through}, \textit{ink fading}, noda, dan \textit{noise} untuk menghasilkan citra terdegradasi yang merepresentasikan kondisi dokumen arsip autentik.
...
Validasi kontribusi komponen dan identifikasi konfigurasi optimal dilakukan melalui eksperimen terkontrol yang didokumentasikan menggunakan MLflow. Eksperimen mencakup: (1) studi ablasi sistematis untuk evaluasi kontribusi setiap komponen fungsi loss, (2) eksplorasi diskriminator dual-modal vs single-modal dengan generator identik, dan (3) optimasi \textit{hyperparameter} melalui pencarian empiris bertahap. Detail protokol evaluasi dan hasil studi ablasi disajikan lengkap pada Tahap 5 (Section 3.2.5).
...
\textit{Ablasi Arsitektur Diskriminator:} Dua eksperimen dilakukan dengan generator identik namun arsitektur diskriminator berbeda: (1) Diskriminator CNN single-modal (\textit{baseline} arsitektur), (2) Diskriminator dual-modal CNN+BiLSTM (arsitektur yang diusulkan). Perbandingan ini mengisolasi kontribusi spesifik komponen dual-modal terhadap kualitas restorasi. Evaluasi dilakukan pada \textit{validation set} ($n$=710) untuk iterasi cepat, dengan verifikasi generalisasi pada \textit{test set} ($n$=712) untuk konfigurasi optimal.
...
Mengevaluasi hipotesis penelitian yang diajukan di Bab I melalui pengujian statistik formal. \textit{Hipotesis Utama (H1)} menguji efektivitas restorasi dengan membandingkan CER hasil restorasi vs \textit{baseline} terdegradasi menggunakan \textit{independent samples t-test} dengan tingkat signifikansi $\alpha = 0.05$. Hipotesis Spesifik Komponen divalidasi melalui studi ablasi: H2\textsubscript{A} mengukur kontribusi diskriminator dual-modal vs single-modal, H2\textsubscript{B} mengukur stabilitas konvergensi \textit{frozen recognizer} melalui analisis varians \textit{loss} (\textit{epoch} 40--50), H2\textsubscript{C} memvalidasi pelestarian kualitas visual tanpa \textit{trade-off} keterbacaan melalui analisis PSNR/SSIM vs CER. Untuk mengoreksi \textit{multiple comparisons} pada hipotesis spesifik (H2\textsubscript{A}, H2\textsubscript{B}, H2\textsubscript{C}), digunakan koreksi Bonferroni dengan $\alpha = 0.0167$ (0.05/3). \textit{Effect size} dihitung menggunakan Cohen's \textit{d} untuk mengukur magnitude perbedaan di luar signifikansi statistik. Kriteria penerimaan hipotesis: \textit{p-value} $<$ $\alpha$ dan \textit{effect size} minimal sedang ($d \geq 0.5$).
...
Kolaborasi dengan Arsip Nasional Republik Indonesia (ANRI) difasilitasi melalui lokakarya teknis, dokumentasi praktis, dan peluang riset lanjutan untuk mendukung adopsi kerangka kerja dalam alur kerja preservasi digital dan digitalisasi dokumen historis.
