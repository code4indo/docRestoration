...
Bab ini menguraikan pendekatan penelitian yang mengadopsi Design Science Research Methodology (DSRM) sebagai kerangka kerja sistematis. Metodologi ini dipilih karena kesesuaiannya untuk merancang dan mengevaluasi artefak penelitian, yaitu framework restorasi dokumen berbasis GAN dengan Diskriminator Dual-Modal. Implementasi teknisnya sendiri berjalan dalam siklus deep learning yang iteratif, di mana setiap iterasi didasarkan pada bukti empiris dan didokumentasikan secara sistematis untuk mencapai tujuan penelitian.
\vspace{1em}
...
\item Orientasi Solusi: DSRM berfokus pada penciptaan artefak inovatif untuk menyelesaikan masalah praktis yang teridentifikasi. Dalam konteks penelitian ini, artefak yang dikembangkan adalah \textit{framework} GAN dengan Diskriminator Dual-Modal yang secara eksplisit dioptimalkan untuk meningkatkan keterbacaan dokumen oleh sistem HTR.

\item Evaluasi Berbasis Bukti: Setiap iterasi desain dievaluasi secara kuantitatif menggunakan metrik yang terukur (PSNR, SSIM, CER, WER), memastikan bahwa penyempurnaan dilakukan berdasarkan data empiris bukan asumsi.

\item Siklus Iteratif: Kerangka kerja ini memfasilitasi perbaikan berkelanjutan melalui putaran umpan balik (\textit{feedback loop}) antara tahap evaluasi dan desain, yang sangat sesuai dengan karakteristik pengembangan model \textit{deep learning} yang memerlukan eksperimen dan \textit{tuning} berulang.

\item Komunikasi dan Diseminasi: DSRM menekankan pentingnya mendokumentasikan dan mengkomunikasikan hasil penelitian kepada komunitas ilmiah, baik melalui publikasi akademis maupun berbagi kode sumber untuk reproduktifitas.
\end{enumerate}
...
\item Diskriminator Dual-Modal: Merancang arsitektur diskriminator yang mampu mengevaluasi kualitas gambar dari dua perspektif simultan: realisme visual (\textit{visual realism}) dan koherensi teks (\textit{text coherence}).

\item Fungsi Loss Berorientasi HTR: Mengintegrasikan sinyal loss dari model HTR secara langsung ke dalam fungsi loss Generator, menciptakan optimasi ganda.

\item Filosofi \textit{Recognizer Terbekukan:} Menggunakan \textit{recognizer} dengan bobot yang dibekukan sebagai evaluator objektif dengan kualitas metrik yang stabil.

\item \textit{Pipeline Degradasi Sintetis:} Mengembangkan \textit{pipeline} degradasi sintetis yang realistis untuk menghasilkan pasangan pelatihan (\textit{training pairs}) dari dokumen bersih.
\end{enumerate}
...
Precision Policy & Pure FP32 & (NO mixed precision) \\ \hline
...
\paragraph{Curriculum-Aware Early Stopping:}
Penelitian ini mengimplementasikan strategi early stopping yang curriculum-aware untuk mencegah premature termination selama fase kritis pembelajaran. Early stopping monitoring baru aktif setelah curriculum learning (warmup + annealing) selesai pada epoch ke-3, dengan patience counter yang dibekukan selama fase curriculum. Sistem ini memonitor metrik validasi (PSNR, CER, atau Combined Score) dan menghentikan training jika tidak ada improvement sebesar min\_delta selama patience epochs berturut-turut. Fitur restore\_best\_weights memastikan model dikembalikan ke state optimal jika terjadi early stopping, dengan checkpoint best model disimpan secara terpisah dari main checkpoints untuk menghindari overwrite akibat max\_to\_keep limitation. Pendekatan ini mengombinasikan efisiensi komputasi dengan proteksi terhadap fase pembelajaran penting.
...
\item PSNR (Peak Signal to Noise Ratio): Mengukur kesamaan piksel dengan ground truth. Target: $>$ 35 dB
\item SSIM (Structural Similarity Index): Mengukur kesamaan struktural. Target: $>$ 0.95
\end{itemize}

\textit{Metrik Text Readability:}
\begin{itemize}[leftmargin=*, nosep]
\item CER (Character Error Rate): Rasio karakter yang salah dikenali. Target: Penurunan minimal 30\% vs baseline
\item WER (Word Error Rate): Rasio kata yang salah dikenali. Target: Penurunan signifikan vs baseline
\end{itemize}
...
Menyiapkan naskah untuk publikasi di wadah ilmiah dengan target jurnal IEEE Transactions on Pattern Analysis and Machine Intelligence (TPAMI) atau International Journal on Document Analysis and Recognition (IJDAR).
...
\textit{Optimizer} & Adam (beta\_1=0.5) & SGD (momentum=0.9) \\ \hline
\textit{Learning Rate} & $2 \times 10^{-4}$ (base) & $5 \times 10^{-4}$ (base) \\ \hline
LR Schedule & Cosine Decay (opsional) & Cosine Decay (opsional) \\ \hline
\textit{Batch Size} & 8 (production), 2 (HPO) & 8 (production), 2 (HPO) \\ \hline
\textit{Loss Function} & Multi-komponen (4 komponen) & \textit{Binary cross-entropy} \\ \hline
Bobot \textit{Loss} (Base) & Pixel: 120.0, RecFeat: 80.0, Adv: 2.5 & - \\ \hline
\textit{CTC Loss} & Bobot 10.0 dengan curriculum (0→10 epoch 1-3) & Gradien ke Generator, Recognizer frozen \\ \hline
Adaptive Balancing & GradNorm-based (target CTC ratio: 20\%) & - \\ \hline
...
Konfigurasi ini dirancang dengan pertimbangan berikut: (1) \textit{learning rate} yang berbeda untuk \textit{generator} (Adam, 2×10$^{-4}$) dan diskriminator (SGD, 5×10$^{-4}$) dengan opsional Cosine Decay schedule untuk konvergensi bertahap, (2) bobot \textit{loss} base ditentukan melalui eksperimen sistematis dengan adaptive balancing menggunakan GradNorm untuk penyesuaian dinamis, (3) \textit{gradient clipping} untuk stabilitas numerik, dan (4) \textit{label smoothing} untuk regularisasi diskriminator.

\paragraph{Adaptive Loss Balancing dengan GradNorm:}
Untuk mengatasi tantangan dalam menyeimbangkan kontribusi CTC loss (berorientasi HTR) dan visual losses (pixel, perceptual, adversarial, recognition feature), penelitian ini mengimplementasikan mekanisme adaptive loss balancing berbasis GradNorm. Sistem ini secara dinamis menyesuaikan bobot relatif antara dua kelompok loss: (1) CTC loss untuk keterbacaan teks, dan (2) visual losses untuk kualitas visual. Target rasio CTC ditetapkan pada 20\% dari total kontribusi loss, dengan penyesuaian otomatis berdasarkan magnitude gradien relatif. Mekanisme ini hanya aktif setelah fase curriculum learning selesai untuk menghindari interferensi dengan warmup dan annealing schedule. Adaptive balancing ini mencegah dominasi CTC loss yang dapat menyebabkan degradasi kualitas visual, sekaligus memastikan Generator tetap dioptimalkan untuk keterbacaan HTR.

\paragraph{Peran CTC Loss dalam Optimasi Generator:}
CTC \textit{loss} dalam penelitian ini digunakan sebagai komponen loss function Generator dan gradiennya di-backpropagate ke Generator untuk mengoptimalkan keterbacaan teks secara langsung. Namun, perlu ditekankan bahwa Recognizer tetap frozen (bobot tidak di-update) sehingga gradien CTC loss hanya mengalir ke Generator, bukan ke Recognizer. Implementasi ini memungkinkan Generator belajar menghasilkan output yang lebih mudah dibaca oleh sistem HTR tanpa memodifikasi Recognizer itu sendiri.
...
Layer Ekstraksi & Layer \texttt{proj\_ln} (output CNN backbone sebelum Transformer encoder), dengan dimensi [batch, 128, 512] untuk setiap sampel \\ \hline
Fungsi Loss & Mean Squared Error (MSE) antara feature maps dari gambar terrestorasi vs. gambar ground truth: 
...
Integrasi dalam Training & RecFeat loss dikombinasikan dengan adversarial dan pixel loss melalui weighted sum. Bobot optimal ditentukan melalui Bayesian Optimization (Bagian \ref{subsubsec:loss-weight-hpo}) \\ \hline
\end{tabular}
\end{table}

Alur Komputasi Recognition Feature Loss:

Implementasi komputasi dilakukan dalam langkah-langkah berikut untuk memastikan efisiensi dan stabilitas numerik:

\begin{enumerate}[leftmargin=*, nosep]
\item Ekstraksi Fitur GT: Pada awal training batch, feature maps dari gambar ground truth diekstrak menggunakan recognizer frozen dan di-cache untuk menghindari komputasi berulang
\item Forward Pass Generator: Generator menerima gambar terdegradasi dan menghasilkan gambar restored
\item Ekstraksi Fitur Restored: Gambar restored dilewatkan ke recognizer frozen untuk mengekstrak feature maps dengan dimensi yang sama
\item Perhitungan MSE: MSE dihitung antara feature maps restored dan GT, dinormalisasi terhadap dimensi (batch × sequence × features)
\item Weighted Integration: RecFeat loss dikombinasikan dengan komponen loss lain: $\mathcal{L}_{\text{Total}} = w_{\text{adv}} \mathcal{L}_{\text{adv}} + w_{\text{pix}} \mathcal{L}_{\text{pix}} + w_{\text{rec}} \mathcal{L}_{\text{RecFeat}}$
\item Backpropagation: Gradien total di-backpropagate hanya ke Generator, recognizer tetap frozen sepanjang pelatihan GAN
\end{enumerate}
...
\begin{itemize}[leftmargin=*, nosep]
\item Optimasi Langsung untuk HTR: Dengan mencocokkan representasi internal HTR model, Generator belajar menghasilkan gambar yang secara eksplisit optimal untuk sistem HTR
\item Stabilitas Pelatihan: MSE pada fitur kontinu lebih stabil daripada cross-entropy pada output diskrit, memberikan gradien yang lebih smooth
\item Gradien Informatif: Feature-level matching memberikan signal pembelajaran yang lebih kaya daripada pixel-level matching, karena merepresentasikan fitur-fitur yang telah dipelajari untuk HTR
\item Keseimbangan Visual-Keterbacaan: RecFeat loss mengoptimalkan keterbacaan tanpa mengorbankan kualitas visual, karena bekerja pada representasi internal yang kaya informasi
\item Kompatibilitas Multi-Objektif: RecFeat loss kompatibel dengan pixel loss, adversarial loss, dan CTC loss dalam framework multi-objektif, memberikan pembelajaran yang komplementer
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Tidak Sistematis: Tidak ada jaminan konvergensi ke konfigurasi optimal
\item Tidak \textit{Reproducible:} Sulit direplikasi oleh peneliti lain
\item Tidak Efisien: Membutuhkan banyak eksperimen manual yang mahal secara komputasi
\item Bias Subjektif: Keputusan dipengaruhi oleh intuisi peneliti, bukan data empiris
\end{itemize}
...
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Pelatihan: Model dilatih selama 2 \textit{epoch} dengan konfigurasi bobot \textit{loss} yang di-\textit{sample} oleh TPE \textit{sampler}
\item Evaluasi: Metrik (PSNR, SSIM, CER, WER) dihitung pada \textit{validation set}
\item \textit{Scoring:} Fungsi objektif dihitung dan dikembalikan ke \textit{optimizer} Optuna
\item Iterasi: TPE \textit{sampler} menggunakan hasil \textit{trial} sebelumnya untuk menyarankan konfigurasi berikutnya
\end{enumerate}
...
\begin{itemize}[leftmargin=*, nosep]
\item Analisis Korelasi: Konfigurasi terbaik pada HPO tetap terbaik pada pelatihan penuh (validasi empiris)
\item Konsistensi Pola: Pola ``\textit{adv} tinggi $\rightarrow$ \textit{collapse}'' konsisten di semua durasi pelatihan
\item Dukungan Literatur: Studi terkait menunjukkan 2-5 \textit{epoch} cukup untuk peringkat \textit{hyperparameter}
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item \textit{Database Studi Optuna}: Tersimpan dalam \textit{database} SQLite (\texttt{hpo\_study.db})
\item Pelacakan MLflow: Semua 29 \textit{trial} terekam dengan metrik, parameter, dan artefak
...
\textit{Connectionist Temporal Classification} (CTC) \textit{loss} berfungsi sebagai metrik evaluasi keterbacaan teks dalam \textit{framework} GAN-HTR. Perlu ditekankan bahwa CTC \textit{loss} TIDAK di-backpropagate ke Generator, melainkan digunakan sebagai \textit{monitoring metric} untuk mengukur kualitas keterbacaan hasil restorasi. Implementasi CTC \textit{loss} memerlukan perhatian khusus terhadap stabilitas numerik agar metrik monitoring akurat dan konsisten.
...
\hline
Kategori & Metrik & Target & Interpretasi \\ \hline
...
H$_0$ (Hipotesis Nol): Tidak ada perbedaan signifikan antara metode yang diusulkan dan \textit{baseline} dalam hal keterbacaan teks (CER).

H$_1$ (Hipotesis Alternatif): Metode yang diusulkan menghasilkan CER yang signifikan lebih rendah dibandingkan \textit{baseline} dengan tingkat kepercayaan 95\%.
...
Justifikasi Desain Ablation:
Desain ablation study disusun secara bertahap untuk mengisolasi kontribusi setiap komponen inovatif:
\begin{itemize}[leftmargin=*, nosep]
\item Baseline: GAN konvensional dengan pixel loss dan adversarial loss saja, tanpa guidance berorientasi HTR
\item + RecFeat Loss: Menambahkan recognition feature loss untuk mengukur dampak guidance dari representasi internal recognizer frozen
\item + CTC Monitoring: Menambahkan CTC loss sebagai metrik monitoring (tidak di-backprop) untuk pelacakan keterbacaan
\item + Dual-Modal Discriminator: Menambahkan input teks ke discriminator untuk realisme konten tekstual
\item Full Framework: Kombinasi lengkap dengan HPO-tuned loss weights untuk optimasi multi-objektif
\end{itemize}

\paragraph{Variabel Kontrol:}
\begin{itemize}[leftmargin=*, nosep]
\item Dataset: Sama untuk semua ablation configurations
\item Training Parameters: Learning rate, batch size, epochs
\item Evaluation Metrics: PSNR, SSIM, CER, WER
\item Random Seeds: Sama untuk reproduktifitas
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item H$_0$: $\mu_{proposed} = \mu_{baseline}$ (no significant difference)
\item H$_1$: $\mu_{proposed} < \mu_{baseline}$ (proposed is significantly better)
\end{itemize}

\paragraph{Statistical Tests:}
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Uji Normalitas: Shapiro-Wilk untuk menguji normalitas distribusi
\item \textit{Paired t-test:} Untuk data terdistribusi normal
\item Wilcoxon \textit{Signed-Rank:} Untuk data tidak normal
\item Ukuran Efek: Cohen's d untuk besaran peningkatan
\end{enumerate}

\paragraph{Multiple Comparison Correction:}
\begin{itemize}[leftmargin=*, nosep]
\item Bonferroni Correction: $\alpha_{corrected} = \frac{\alpha}{n}$ (dimana $n$ = jumlah comparisons)
\item False Discovery Rate: Benjamini-Hochberg procedure
\end{itemize}

\paragraph{Confidence Intervals:}
\begin{itemize}[leftmargin=*, nosep]
\item CI 95\%: Untuk perbedaan rata-rata antara metode
\item \textit{Bootstrap CI:} 1000 pengambilan sampel ulang untuk estimasi yang kokoh
\end{itemize}

\subsubsection{Cross-Validation Strategy}
\paragraph{K-Fold Cross Validation:}
\begin{itemize}[leftmargin=*, nosep]
\item K = 5: Validasi silang 5-\textit{fold} untuk estimasi performa yang kokoh
\item Pengambilan Sampel Berlapis: Menjaga distribusi kelas di setiap \textit{fold}
\item \textit{Nested CV:} Putaran dalam untuk \textit{hyperparameter tuning}, putaran luar untuk evaluasi
\end{itemize}

\subparagraph{Temporal Validation:}
\begin{itemize}[leftmargin=*, nosep]
\item Pembagian Berbasis Waktu: Pelatihan pada dokumen lama, pengujian pada dokumen baru
\item Uji Generalisasi: Mengukur kemampuan generalisasi ke domain yang belum pernah dilihat
\end{itemize}
...
\paragraph{Publikasi Ilmiah:}
Menyiapkan naskah untuk publikasi ilmiah dengan target jurnal IEEE Transactions on Pattern Analysis and Machine Intelligence (TPAMI) atau International Journal on Document Analysis and Recognition (IJDAR).
...
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Dekomposisi Fungsional: Sistem dipecah berdasarkan fungsi utama (generasi gambar, evaluasi kualitas, pengenalan teks, optimasi) untuk mengidentifikasi batas-batas modul yang alami.

\item Analisis Antarmuka: Identifikasi titik komunikasi antar komponen untuk mendefinisikan kontrak antarmuka yang jelas dan meminimalkan ketergantungan.

\item Pemetaan Ketergantungan: Pembuatan diagram ketergantungan untuk memvisualisasikan relasi antar modul dan memastikan tidak ada ketergantungan siklik yang dapat menghambat modularitas.

\item Definisi Lapisan Abstraksi: Penetapan hierarki abstraksi untuk memisahkan logika bisnis tingkat tinggi dari detail implementasi tingkat rendah.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Konfigurasi Hierarkis: Sistem konfigurasi berlapis dengan konfigurasi dasar yang dapat diwariskan dan ditimpa. Pendekatan ini memungkinkan definisi parameter umum pada tingkat dasar dan penyesuaian spesifik pada tingkat eksperimen.

\item Penimpaan Berbasis Lingkungan: Konfigurasi dapat ditimpa melalui variabel lingkungan untuk memfasilitasi \textit{deployment} pada berbagai infrastruktur tanpa modifikasi kode.

\item Validasi Waktu Eksekusi: Validasi otomatis konfigurasi pada waktu eksekusi untuk mendeteksi kesalahan konfigurasi sebelum pelatihan dimulai, menghemat waktu dan sumber daya komputasi.

\item Manajemen Nilai Bawaan: Setiap parameter memiliki nilai bawaan yang wajar berdasarkan praktik terbaik, mengurangi beban konfigurasi manual dan memfasilitasi penggunaan \textit{framework} oleh pengguna baru.
\end{enumerate}
...
\begin{itemize}[leftmargin=*, nosep]
\item Arsitektur \textit{Plugin:} Mekanisme registrasi memungkinkan penambahan komponen kustom (misalnya arsitektur \textit{generator} baru) tanpa modifikasi kode inti, memfasilitasi eksperimen dengan variasi arsitektur.

\item Titik Ekstensi: Titik-titik ekstensi yang telah ditentukan (\textit{hook points}) memungkinkan penyisipan logika kustom pada tahap kritis (misalnya sebelum/sesudah setiap \textit{epoch}) untuk keperluan pemantauan atau modifikasi perilaku.

\item Kompatibilitas Mundur: Strategi untuk memastikan kompatibilitas dengan versi sebelumnya, melindungi investasi penelitian yang ada dan memfasilitasi reproduktifitas jangka panjang.

\item Manajemen Versi: Penerapan \textit{semantic versioning} untuk rilis \textit{framework}, memberikan sinyal yang jelas tentang perubahan yang dapat merusak kompatibilitas (\textit{breaking changes}) versus perbaikan yang kompatibel.
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Penyimpanan Otomatis: Setiap eksperimen menyimpan snapshot lengkap konfigurasi yang digunakan, termasuk parameter yang diwariskan dan yang ditimpa.

\item Integrasi dengan MLflow: Konfigurasi dicatat sebagai parameter eksperimen dalam MLflow, memungkinkan penelusuran dan perbandingan konfigurasi antar eksperimen.

\item Versi Konfigurasi: Setiap perubahan pada konfigurasi dasar diberikan nomor versi untuk pelacakan evolusi parameter sepanjang penelitian.

\item Validasi Konsistensi: Sistem validasi memastikan bahwa kombinasi parameter valid dan kompatibel sebelum eksekusi, mencegah kesalahan konfigurasi yang dapat menyebabkan kegagalan pelatihan.
\end{itemize}
...
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Pengujian Arsitektur Model: Verifikasi dimensi keluaran, jumlah parameter, dan aliran gradien untuk memastikan arsitektur diimplementasikan dengan benar. Pengujian mencakup validasi bahwa dimensi tensor sesuai dengan spesifikasi desain dan gradien dapat mengalir mundur tanpa hambatan.

\item Pengujian {\textit{Pipeline} Data:} Validasi proses pemuatan data, praproses, dan augmentasi untuk memastikan integritas data. Pengujian mencakup verifikasi format data, normalisasi nilai, dan konsistensi transformasi augmentasi.

\item Pengujian Fungsi \textit{Loss:} Verifikasi perhitungan fungsi \textit{loss} multi-komponen dan pembaruan \textit{optimizer} untuk memastikan gradien dihitung dengan benar. Pengujian mencakup validasi bobot \textit{loss}, stabilitas numerik, dan konvergensi \textit{optimizer}.

\item Pengujian Integrasi Komponen: Validasi interaksi antar komponen dalam \textit{pipeline} pelatihan ujung-ke-ujung untuk mendeteksi inkompatibilitas antarmuka atau masalah integrasi.
\end{enumerate}
...
\begin{itemize}[leftmargin=*, nosep]
\item Pengujian Awal (\textit{Smoke Test):} Validasi cepat menggunakan \textit{dataset} kecil dan \textit{epoch} terbatas untuk memverifikasi bahwa \textit{pipeline} lengkap dapat dijalankan tanpa kesalahan kritis. Pengujian ini mendeteksi masalah struktural sebelum pelatihan skala penuh dimulai.

\item Pengujian Komponen Modular: Validasi independen setiap modul utama (\textit{generator}, diskriminator, \textit{recognizer}) untuk memastikan fungsionalitas yang benar sebelum integrasi penuh.

\item Pengujian \textit{Pipeline Ujung-ke-Ujung:} Validasi lengkap dari input data mentah hingga keluaran model akhir untuk memverifikasi bahwa seluruh \textit{pipeline} berfungsi sebagai sistem terintegrasi.

\item Pengujian Performa: Pengukuran waktu inferensi dan penggunaan memori untuk memastikan efisiensi komputasi memenuhi target yang ditetapkan (inferensi $<$ 15 detik per gambar).
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Validasi Konvergensi: Pemantauan stabilitas pelatihan melalui analisis variansi \textit{loss} dan tren konvergensi untuk mendeteksi \textit{mode collapse}, ledakan gradien, atau ketidakstabilan lainnya.

\item Validasi Kualitas Minimum: Verifikasi bahwa metrik kualitas memenuhi ambang batas minimum yang ditetapkan (PSNR $>$ 35 dB, SSIM $>$ 0.95, CER menunjukkan perbaikan signifikan) sebelum model diterima untuk evaluasi lebih lanjut.

\item Validasi Performa Komputasi: Pengukuran \textit{benchmark} waktu inferensi untuk memastikan model dapat beroperasi dalam batasan waktu yang praktis untuk aplikasi nyata.

\item Validasi Robustitas: Pengujian kemampuan model menangani variasi input yang tidak terduga atau berada di luar distribusi data pelatihan untuk mengukur generalisasi dan ketahanan.
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Deteksi Lonjakan \textit{Loss:} Pemantauan nilai \textit{loss} ekstrem yang mengindikasikan ketidakstabilan numerik atau konfigurasi yang salah.

\item Deteksi Gradien Abnormal: Pemantauan norma gradien untuk mendeteksi gradien yang meledak atau menghilang yang dapat menghambat pelatihan.

\item Deteksi Distribusi Keluaran: Analisis distribusi statistik keluaran model untuk mendeteksi \textit{mode collapse} atau degradasi kualitas generasi.
\end{itemize}
...
\begin{enumerate}[label=\arabic*., leftmargin=*, nosep]
\item Ekstraksi Fitur Pra-latih: Model yang telah dilatih pada dataset penelitian dapat digunakan sebagai ekstraktor fitur untuk domain target baru. Bobot lapisan konvolusi dalam \textit{generator} dan \textit{recognizer} mengandung representasi fitur umum yang dapat ditransfer.

\item Penyetelan Halus Bertahap (\textit{Fine-tuning):} Metodologi penyetelan halus berlapis memungkinkan adaptasi bertahap ke domain target. Lapisan awal (fitur \textit{low-level}) dibekukan, sementara lapisan akhir (fitur \textit{high-level}) disesuaikan dengan data domain baru.

\item Adaptasi Domain: Teknik adaptasi domain diterapkan untuk menangani pergeseran distribusi data (\textit{domain shift}) antara domain sumber dan target. Ini mencakup strategi regularisasi dan augmentasi data yang disesuaikan dengan karakteristik domain target.

\item Pelatihan Multi-Domain: Untuk meningkatkan generalisasi, \textit{framework} mendukung pelatihan simultan pada beberapa domain dokumen, memungkinkan model belajar representasi fitur yang lebih umum dan kokoh terhadap variasi.
\end{enumerate}
...
\begin{itemize}[leftmargin=*, nosep]
\item Evaluasi Lintas Domain: Pengujian model pada domain dokumen yang tidak terlihat selama pelatihan untuk mengukur kemampuan generalisasi. Ini mencakup dokumen dengan periode waktu berbeda, gaya tulisan berbeda, dan jenis degradasi yang bervariasi.

\item Analisis Robustitas: Evaluasi performa terhadap variasi degradasi yang tidak ada dalam data pelatihan untuk menguji ketahanan model terhadap kondisi yang tidak terduga.

\item Uji Skalabilitas: Validasi kemampuan \textit{framework} untuk menangani dokumen dengan ukuran dan kompleksitas yang bervariasi, dari dokumen sederhana hingga dokumen dengan layout kompleks.
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Dokumentasi Arsitektural: Spesifikasi lengkap arsitektur model, termasuk dimensi lapisan, fungsi aktivasi, dan skema koneksi untuk memfasilitasi reimplementasi oleh peneliti lain.

\item Panduan Eksperimen: Dokumentasi prosedur eksperimental yang detail, termasuk konfigurasi \textit{hyperparameter}, strategi pelatihan, dan kriteria evaluasi untuk memungkinkan replikasi hasil.

\item Praktik Terbaik (\textit{Best Practices):} Kompilasi pembelajaran dan rekomendasi dari proses pengembangan untuk memandu peneliti lain dalam adaptasi dan pengembangan \textit{framework}.

\item Penanganan Kasus Khusus: Dokumentasi masalah umum yang ditemui selama pengembangan beserta solusinya untuk membantu peneliti menghindari kesalahan yang sama dan mempercepat adopsi.
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep]
\item Kode Sumber Terbuka: Publikasi implementasi lengkap dengan lisensi \textit{open-source} untuk memfasilitasi adopsi, verifikasi, dan pengembangan lebih lanjut oleh komunitas penelitian.

\item Model Pra-latih: Penyediaan bobot model yang telah dilatih untuk memungkinkan peneliti memulai dari \textit{baseline} yang kuat tanpa perlu melatih dari awal, menghemat sumber daya komputasi.

\item Dataset dan \textit{Benchmark:} Berbagi \textit{dataset} sintetis dan protokol evaluasi standar untuk memfasilitasi perbandingan yang adil dengan metode masa depan dan mendorong kemajuan bidang penelitian.
\end{itemize}
...
% \begin{enumerate}[leftmargin=*, nosep]
% \item Reproduktifitas: Semua eksperimen terlacak dengan MLflow, kode tersimpan dalam repositori Git dengan \textit{commit hash} yang terdokumentasi, memungkinkan peneliti lain untuk mereplikasi hasil penelitian secara akurat.

% \item Transparansi: Setiap keputusan desain dan konfigurasi didokumentasikan dengan justifikasi berbasis data empiris, memastikan proses penelitian dapat diaudit dan diverifikasi oleh komunitas ilmiah.

% \item Ketelitian: Evaluasi dilakukan menggunakan metrik standar industri (PSNR, SSIM, CER, WER) dan pengujian signifikansi statistik yang ketat untuk memvalidasi hipotesis penelitian secara objektif.

% \item Generalisasi: Metodologi reusabilitas dan validasi lintas domain memastikan \textit{framework} dapat diadaptasi untuk berbagai jenis dokumen historis dan skenario aplikasi yang beragam.

% \item Dampak: Hasil penelitian dikomunikasikan melalui publikasi ilmiah di \textit{venue} bereputasi tinggi dan berbagi kode sumber terbuka untuk manfaat komunitas penelitian dan praktisi di bidang restorasi dokumen.
% \end{enumerate}
