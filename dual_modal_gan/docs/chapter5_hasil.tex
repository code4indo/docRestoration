...
Studi ablasi dilakukan untuk memahami kontribusi setiap komponen fungsi \textit{loss} secara sistematis dan empiris. Pendekatan ablasi inkremental digunakan, di mana komponen \textit{loss} ditambahkan secara bertahap untuk mengisolasi dampak individual terhadap kualitas restorasi.

\textbf{Protokol Eksperimen:}
Lima konfigurasi eksperimen dilatih dengan penambahan komponen \textit{loss} secara inkremental:
\begin{enumerate}[nosep]
    \item \textbf{Eksperimen 01 (Baseline U-Net)}: Hanya $\mathcal{L}_{\text{pixel}}$ (L1) dengan bobot $\lambda=50$
    \item \textbf{Eksperimen 02 (+Adversarial)}: Penambahan $\mathcal{L}_{\text{adv}}$ dengan bobot $\lambda=3$
    \item \textbf{Eksperimen 03 (+Perceptual)}: Penambahan $\mathcal{L}_{\text{perc}}$ (VGG-19) dengan bobot $\lambda=1$
    \item \textbf{Eksperimen 04 (+CTC)}: Penambahan $\mathcal{L}_{\text{ctc}}$ untuk kesadaran HTR dengan bobot $\lambda=0.15$
    \item \textbf{Eksperimen 05 (+RecFeat)}: Penambahan $\mathcal{L}_{\text{rec-feat}}$ dengan bobot $\lambda=8$
\end{enumerate}

Setiap eksperimen dilatih selama 15 \textit{epoch} dengan \textit{hyperparameter} identik (\textit{learning rate}, \textit{batch size}, \textit{optimizer}) untuk mengisolasi dampak komponen \textit{loss}. Konfigurasi \textit{dataset split} identik untuk semua eksperimen (train 70\%, validasi 15\%, uji 15\%, seed=42). Detail konfigurasi parameter setiap eksperimen dijelaskan pada Bab IV Subbab~\ref{subsubsec:spesifikasi-ablasi}.

\textbf{Metrik Evaluasi:} 
Tiga metrik utama digunakan untuk menilai dampak setiap komponen: (1) PSNR untuk kualitas visual piksel, (2) SSIM untuk kesamaan struktural, dan (3) CER untuk keterbacaan teks HTR. Evaluasi dilakukan pada \textit{validation set} ($n$=710 citra) yang digunakan untuk pemilihan \textit{checkpoint} terbaik, mengikuti praktik standar studi ablasi untuk iterasi cepat dan mencegah kontaminasi \textit{test set}.
...
\paragraph{CTC Loss (Eksperimen 04)} mengaktifkan kapabilitas kesadaran HTR, mencapai CER 29.66\% sambil mempertahankan kualitas visual yang kompetitif (PSNR 24.76 dB, hanya -0.10 dB dari maksimum). Dalam konteks ablasi 15 \textit{epoch}, ini adalah konfigurasi \textbf{optimal untuk evaluasi cepat}, mencapai keseimbangan Pareto antara kualitas visual dan keterbacaan teks tanpa \textit{overhead} komponen kelima.
...
\begin{enumerate}[nosep]
    ...
    \item Studi ablasi 15 \textit{epoch} efektif untuk mengukur \textbf{kontribusi relatif} setiap komponen, bukan performa absolut
...
Keunggulan utama \textit{frozen recognizer} bukan pada efisiensi komputasi, melainkan pada \textbf{stabilitas pelatihan} dan \textbf{preservasi kemampuan HTR}. Hal ini terbukti dari CER yang stabil (31.63\%) dibandingkan \textit{joint training} yang mengalami \textit{kelupaan katastropik} total (CER 100\%). Untuk aplikasi praktis restorasi dokumen historis, stabilitas dan akurasi HTR jauh lebih kritis daripada penghematan waktu atau memori.
...
[Isi akan ditambahkan: sintesis ringkas dari semua hasil utama yang dipaparkan, poin-poin kunci yang menjawab pertanyaan penelitian, highlight temuan penting yang mendukung atau menolak hipotesis, transisi ke bab kesimpulan]
